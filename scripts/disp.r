...
(Fig1a<-ggplot(df3, aes(x=DOY, y=NDVI)) +
  theme_classic(base_size=14) + 
  geom_smooth(method = "loess", size = 1.5, col='darkgreen') +
  xlab("Day of Year") +
  geom_point(data=df3, aes(x=DOY, y=NDVI), color='black', alpha=0.1)+
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  geom_point(data=df3_SP, aes(x=DOY, y=NDVI,color=as.factor(YR)), alpha=0.3, size =2)+
...
Fig1ab<-arrangeGrob(f1A, f1B, ncol=1)
ggsave(Fig1ab, filename ="figs/fig1ab_new.png",device="png",width=7, height = 10)
